...
local function is_transport_belt_blocked(entity)
    if entity.type == "transport-belt" then
        ...
        if not has_sink then
            local line_1_moving = line_1 and storage.utils.get_contents_compat(line_1) and not line_1.can_insert_at_back()
            local line_2_moving = line_2 and storage.utils.get_contents_compat(line_2) and not line_2.can_insert_at_back()
...
local function has_fuel(entity)
    if entity.burner then
        if not entity.burner.currently_burning then
            ...
            for item_name, item_count in pairs(storage.utils.get_contents_compat(fuel_inventory)) do
